  ...
  onFileSelected(event: Event): void {
    ...
    if (input.files) {
      ...
      if (invalidFiles.length > 0) {
        this.handleError(`Some files exceed the 10MB limit: ${invalidFiles.map(f => `${f.name} (${this.formatFileSize(f.size)})`).join(', ')}`);
        this.selectedFiles = [];
        input.value = '';
      ...
      if (totalSize > maxFileSize) {
        this.handleError(`The total file size exceeds the 10MB limit. Total size: ${this.formatFileSize(totalSize)}`);
        this.selectedFiles = [];
        input.value = '';
      }
    }
