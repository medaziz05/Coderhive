...
export class FileStorageService {
  ...
  uploadFile(file: File): Observable<{ filePath: string }> {
    ...
    return this.http.post<{ filePath: string }>(`${this.apiUrl}/attachments`, formData).pipe(
  ...
  deleteFile(filePath: string): Observable<void> {
    return this.http
      .delete<void>(`${this.apiUrl}/attachments/${encodeURIComponent(filePath)}`)
      .pipe(
        catchError(this.handleError), // Handle errors consistently
      );
  }
  ...
  private handleError(error: HttpErrorResponse) {
    ...
    return throwError(() => new Error(error.error?.message || 'File operation failed'));
  }
}
