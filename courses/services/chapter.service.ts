...
export class ChapterService {
  ...
  createChapter(courseId: number, formData: FormData): Observable<Chapter> {
    return this.http.post<Chapter>(`${this.baseUrl}/${courseId}/chapters`, formData).pipe(
  ...
  updateChapter(courseId: number, chapterId: number, formData: FormData): Observable<Chapter> {
    return this.http
      .put<Chapter>(`${this.baseUrl}/${courseId}/chapters/${chapterId}`, formData)
      .pipe(
        catchError(this.handleError), // Catch and handle any errors
      );
  }
  ...
  getChapterDetails(courseId: number, chapterId: number): Observable<ChapterDetails> {
    return this.http
      .get<ChapterDetails>(`${this.baseUrl}/${courseId}/chapters/${chapterId}`)
  ...
  getChapterQuiz(chapterId: number): Observable<QuizDTO> {
    return this.http.get<QuizDTO>(
      `${this.baseUrl}/api/courses/{courseId}/chapters/${chapterId}/quiz`
    );
  }

  //student // 

  getChapterForStudent(courseId: number, chapterId: number, studentId: number): Observable<ChapterDetails> {
    return this.http.get<ChapterDetails>(`${this.baseUrl}/${courseId}/chapters/${chapterId}/student/${studentId}`)
      .pipe(
...
//student 
  ...
  checkStudentMembership(courseId: number, studentId: number): Observable<boolean> {
    return this.http.get<boolean>(
      `${this.baseUrl}/${courseId}/chapters/membership/${studentId}`
    ).pipe(catchError(this.handleError));
  }
